...
Each EMG signal is preprocessed using:
    notch filtering → bandpass filtering → rectification → envelope extraction.
...
t_bar = np.arange(frames) / frame_rate          # 모션캡쳐 시간축


# =========================
# 필터 함수들
# =========================
# Lowpass filter function (4–8 Hz)
...
# =========================
# PLOT (맨 위: BAR_Z, 그 아래 EMG들)
# =========================
n_rows = len(emg_indices) + 1      # 1개는 BAR_Z, 나머지는 EMG
...
for i, idx in enumerate(emg_indices):
    ...
    # 5. Envelope using Low-pass Filter (LPF 4–8Hz)
    b_lp, a_lp = lowpass(6, fs)   # 6 Hz 중심
    ...
    # ------------ PLOT -------------
    ax = plt.subplot(n_rows, 1, i+2)   # +2: 1행은 BAR_Z
...
# =========================
# EXTRA SUBPLOTS — SLICED 11.5 ~ 15.7 sec
# =========================
...
# 새로운 figure 혹은 기존 figure 아래에 추가 가능
plt.figure(figsize=(15, 16))
